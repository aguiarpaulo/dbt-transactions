...
with DAG(
    ...
    run_dbt_staging = BashOperator(
        ...
        bash_command='cd .. && cd transactions && dbt run --select staging',
    )

    # Task to run intermediate models
    run_dbt_intermediate = BashOperator(
        task_id='dbt_run_intermediate',
        bash_command='cd .. && cd transactions && dbt run --select intermediate',
    )

    # Task to run marts models
    run_dbt_marts = BashOperator(
        task_id='dbt_run_marts',
        bash_command='cd .. && cd transactions && dbt run --select marts',
    )
